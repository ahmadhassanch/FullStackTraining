...
// import * as DD from './dropzone';
// export const Dropzone = DD.Dropzone;

// declare var module: any;
// export const Dropzone: any = require("./dropzone.js");
...
export class DropzoneDirective implements OnInit, OnDestroy, OnChanges // DoCheck
{
  ...
  ngOnInit(): void {
    ...
    DropzoneEvents.forEach((eventName) => {
      this.instance.on(eventName.toLowerCase(), (...args) => {
        ...
        let dict = {dropzone: this.instance, data: args}
        this.zone.run(() => {
          this[`DZ_${eventName.toUpperCase()}`].emit(dict);
        });
      });
    });
